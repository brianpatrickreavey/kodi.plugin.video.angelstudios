...
def load_test_credentials():
    """Load test credentials from a file for testing purposes"""
    test_creds_file = './test_credentials.json'
    if os.path.exists(test_creds_file):
        import json
        try:
            with open(test_creds_file, 'r') as f:
                creds = json.load(f)
                return creds.get('username'), creds.get('password')
        except Exception as e:
            print(f"Error loading test credentials: {e}")
    return None, None

def get_angel_session():
    """Get an authenticated Angel Studios session for development"""
    username, password = load_test_credentials()
    asi = angel_studios_interface.AngelStudiosInterface(
        username=username,
        password=password,
        session_file="new_dev_shell_angel_session.pkl",
        logger=stdout_log
    )
    return asi
...
try:
    from . import angel_authentication
    from resources.lib import angel_graphql
    from resources.lib import angel_content
    print("✓ Project modules loaded (angel_studios_authentication, angel_graphql, angel_content)")
    ...
    def test_auth(username=None, password=None):
        ...
        try:
            session = angel_authentication.get_authenticated_session()
            if session and angel_authentication.is_session_valid(session):
        ...
        try:
            session = angel_authentication.get_authenticated_session(username, password)
    ...
    def force_reauth(username=None, password=None):
        ...
        try:
            session_file = angel_authentication.get_session_file()
...
stdout_log = logging.getLogger("new_dev_shell")
logger_handler = logging.StreamHandler()  # Handler for the logger
stdout_log.addHandler(logger_handler)
stdout_log.info("STDOUT logger initialized")


print("\nStarting interactive shell...")
print("-" * 50)

from resources.lib import angel_studios_interface
