...
class TestMainMenu:
    ...
    def test_cache_disabled_bypasses_get_set(self, ui_interface):
        ...
        ui.cache_manager.addon = fresh_addon  # Update cache_manager addon too
    ...
    def test_trace_writer_redacts_and_stores(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ui.ui_helpers.trace_dir = tmpdir
...
class TestContinueWatchingMenu:
            ...
            "episodes": [
                {
                    ...
                    "duration": 3600,
                    "watchPosition": {"position": 1200},
                    "source": {"url": "https://example.com/1.m3u8"},
                    "project": {"slug": "project-1", "name": "Test Show 1"},
                },
                {
                    "guid": "resume-guid-2",
                    "name": "Episode 2",
                    ...
                    "duration": 2400,
                    "watchPosition": {"position": 600},
                    "source": {"url": "https://example.com/2.m3u8"},
                    "project": {"slug": "project-2", "name": "Test Show 2"},
                },
            ],
    ...
    def test_continue_watching_menu_skips_malformed_items(self, ui_interface, mock_xbmc):
        """Test continue_watching_menu processes all episodes in response."""
        ui, logger_mock, angel_interface_mock = ui_interface
        mock_add_item, mock_end_dir, mock_list_item = mock_xbmc
    ...
    def test_continue_watching_menu_failed_episodes_batch(self, ui_interface, mock_xbmc):
        """Test continue_watching_menu when episodes list is empty."""
        ui, logger_mock, angel_interface_mock = ui_interface
        mock_add_item, mock_end_dir, mock_list_item = mock_xbmc

        # Resume watching returns empty episodes
        angel_interface_mock.get_resume_watching.return_value = {
            "episodes": [],
...
class TestUtils:
    ...
    def test_show_error(self, ui_interface, mock_xbmc):
        ...
        with patch("xbmcgui.Dialog") as mock_dialog:
            ...
            logger_mock.error.assert_called_once_with("Error dialog shown: Angel Studios - Test error message")
...
class TestAdditionalCoverage:
    ...
    def test_ensure_trace_dir_exception(self, ui_interface, monkeypatch):
        ...
        assert ui._ensure_trace_dir() is False
        logger_mock.error.assert_any_call(f"Failed to create trace directory {ui.ui_helpers.trace_dir}: boom")
    ...
    def test_trim_trace_files_removes_oldest(self, ui_interface, monkeypatch):
        ...
        with (
            ...
            patch("resources.lib.kodi_ui_helpers.os.path.isfile", return_value=True),
            patch("resources.lib.kodi_ui_helpers.os.path.getmtime", side_effect=[3, 2, 1]),
    ...
    def test_clear_debug_data_success_path(self, ui_interface, monkeypatch, tmp_path):
        ...
        ui.ui_helpers.trace_dir = str(trace_dir)
        ...
        assert ui.clear_debug_data() is True
        logger_mock.info.assert_any_call(f"Cleared 2 trace files from {ui.ui_helpers.trace_dir}")
